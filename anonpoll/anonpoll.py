...
class AnonPoll(commands.Cog):
    """Commands for Anonymous Polls"""

    def __init__(self, bot):
        self.bot = bot
        self.conf = Config.get_conf(self, identifier=1148676844768, force_registration=True)
    ...
    async def apollset(self, ctx: commands.Context):
        """
            Settings for reaction polls
        """

    @apollset.command(name="embed", aliases=["embeds"])
    async def apoll_set_embed(self, ctx: commands.Context):
        """
    ...
    @commands.group()
    @commands.guild_only()
    async def apoll(self, ctx: commands.Context):
        """Commands for setting up reaction polls"""
        pass

    @apoll.command(name="end", aliases=["close"])
    ...
    @apoll.command(name="interactive")
    async def apoll_interactive(self, ctx: commands.Context, channel: discord.TextChannel):
        """
    ...
    @apoll.command(name="new", aliases=["create"])
    async def apoll_create(
        self,
        ...
        *,
        ...
        """
            ...
            `[p]apoll new Is this a poll? Yes;No;Maybe; 2 hours 21 minutes 40 seconds multi-vote`
        """
        if not channel:
