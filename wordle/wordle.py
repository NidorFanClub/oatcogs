...
class Wordle(commands.Cog):
    ...
    async def draw_postgame(self, ctx, target_word, guesses, member: discord.Member):
        ...
        frame.text(xy = ((canvas_width / 2 - statistic_label_width / 2), (2 * canvas_padding + heading_height + statistic_value_height / 2)), text = f"{100 * (float(await self.config.member(member).played()) / await self.config.member(member).played()):.0}", fill = text_color, font = statistic_value, anchor = "mm")
